      ...
      <div className="relative z-10 flex flex-col flex-1">
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-white">Transaction History</h1>
              </div>
          ...
          {!authLoading && !isAuthenticated ? (
            ...
            <>
              {/* Search input */}
              <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 shadow-xl backdrop-blur-md flex items-center justify-between gap-4">
                <div className="relative w-full md:w-80">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition-colors font-mono"
                  />
                </div>
                <div className="text-xs text-slate-400 hidden sm:block font-mono">
                  Showing <strong className="text-slate-200">{filteredOrders.length}</strong> records
                </div>
              </div>
              ...
              {loading && orders.length === 0 ? (
                <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-12 text-center flex flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-sm font-semibold text-slate-300">Loading audit history...</p>
                </div>
                ...
                <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-12 text-center flex flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-sm font-bold text-slate-200">No transaction records found</p>
                          ...
                          return (
                            ...
                            >
                              <td className="py-4 px-4 font-mono">
                                <div className="font-bold text-slate-200">#{ord.id.slice(0, 8)}...</div>
                                {pmt?.reference && (
                                  <div className="text-[10px] text-amber-300/80 truncate max-w-[120px]">
                                    Ref: {pmt.reference}
                                  </div>
                                )}
                              </td>
                              <td className="py-4 px-4 font-bold text-slate-100">
                                ₦{Number(ord.sourceAmount).toLocaleString()} <span className="text-[10px] text-slate-400 font-normal">NGN</span>
                              </td>
                              <td className="py-4 px-4 font-bold text-emerald-400">
                                {Number(ord.destinationAmount).toLocaleString()} <span className="text-[10px] text-emerald-300 font-normal">{ord.destinationAsset}</span>
                              </td>
                              ...
                              <td className="py-4 px-4 font-mono">
                                {txHash ? (
                                  ...
                                  <span className="text-slate-500 text-[11px]">Unsettled</span>
                                )}
                              </td>
                              ...
                              <td className="py-4 px-4 text-right" onClick={(e) => e.stopPropagation()}>
                                <div className="flex items-center justify-end gap-2">
                                  ...
                                  <button
                                    onClick={() => handleOpenDetails(ord.id)}
                                    className="py-1.5 px-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded text-[11px] font-medium transition-all cursor-pointer font-sans"
                                  >
                                    View
                                  </button>
                                </div>
                  ...
                  {total > limit && (
                    <div className="bg-slate-950/80 px-4 py-3 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
                      <div>
                        ...
                        of <span className="font-semibold text-slate-200">{total}</span> records
                      </div>
                      <div className="flex items-center gap-2">
                        <button
